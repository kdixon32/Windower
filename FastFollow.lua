...
local function configured_group()
  return T(settings.group or {})
end

local function save_group(group)
  settings.group = T(group or {})
  config.save(settings, 'all')
end

local function selected_followers(args)
  local self = windower.ffxi.get_mob_by_target('me')
  if not self and not repeated then
    repeated = true
    local selected = #args > 0 and ' '..args:concat(' ') or ''
    windower.send_command('@wait 1; ffo followme'..selected)
    return
  end
  ...
  repeated = false
  if #args > 0 then
    windower.send_ipc_message('followselected '..self.name..' '..args:concat(' '))
  else
...
local function group_followers()
  local self = windower.ffxi.get_mob_by_target('me')
  if not self and not repeated then
    repeated = true
    windower.send_command('@wait 1; ffo group')
    return
  end
  if not self then return end

  local self_name = self.name:lower()
  local group = configured_group()
  local followers = T{}

  for _, name in ipairs(group) do
    if name:lower() ~= self_name then
      followers:append(name)
    end
  end

  if #followers == 0 then
    return windower.add_to_chat(0, 'FastFollow: No configured group members to follow '..self.name..'.')
  end

  repeated = false
  windower.send_ipc_message('followselected '..self.name..' '..followers:concat(' '))
  windower.send_ipc_message('track '..(settings.show and 'on' or 'off'))
end
...
windower.register_event('addon command', function(command, ...)
  ...
  if not command then
    ...
    log('Use "me <name> [name...]" to make only specific characters follow you.')
    log('Use "setgroup <name> [name...]" once, then "group" to make that saved group follow you.')
    log('Use "showgroup" to display the saved follow group.')
  ...
  elseif command == 'followme' or command == 'me' then
    selected_followers(args)
  elseif command == 'setgroup' then
    if #args == 0 then
      return windower.add_to_chat(0, 'FastFollow: Provide one or more character names for the follow group.')
    end

    save_group(args)
    windower.send_ipc_message('setgroup '..args:concat(' '))
    windower.add_to_chat(0, 'FastFollow: Follow group set to '..args:concat(', ')..'.')
  elseif command == 'showgroup' or command == 'listgroup' then
    local group = configured_group()
    windower.add_to_chat(0, #group > 0 and 'FastFollow: Saved follow group: '..group:concat(', ')..'.' or 'FastFollow: No follow group configured.')
  elseif command == 'group' or command == 'team' or command == 'boxes' then
    group_followers()
...
windower.register_event('ipc message', function(msgStr)
  ...
  elseif command == 'follow' then
    ...
    windower.send_ipc_message('following '..following)
    windower.ffxi.follow()
  elseif command == 'followselected' then
    local leader = args:remove(1)
    local self = windower.ffxi.get_player()
    if not self or not args:contains(self.name:lower()) then return end
